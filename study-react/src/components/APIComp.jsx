// 데이터 API를 통해서 값(뉴스 데이터)를 가져오는 컴포넌트
// 데이터 API의 특징
// - 데이터를 가져오기 위해 Ajax를 사용하여 비동기로 값을 가져와서 사용
//   (http객체, fetchAPI, axios)
...
const APIComp = () => {
  ...
  // api의 내용이 많을 시 그 값이 빨리 전달되지 않을 때가 있기에
  // 그 값이 왔을 때 제어하기 위한 state
  const [newsloading, setNewsloading] = useState(false);

  // 시작하자마자 그 값을 가져오기 위해 useEffect에 작성
  ...
  return (
    <div>
       ...
       *
       * news를 조건식으로 넣으면 > 값이 있을 때 : true / 값이 없을 때 : false
       */}
...
// 컴포넌트를 작성해서 사용 >> 그 값을 props로 받아옴
const APIBox = (props) => {
  // props의 값은 객체로 들어오기 때문에,
  // 객체 기호로 감싸서 구조화 할당 사용 / 배열은 배열 기호로 감싸서
