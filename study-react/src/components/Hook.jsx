...
const Hook = () => {
  ...
  const changeCount = () => {
    // setCount가 3번 진행이 되었기 때문에 결과값이 4가 출력되어야한다.
    // 실행한 결과: 2
  ...
  // setCount를 3번 실행했을 때 +3이 되게 하기위한 함수
  ...
  // 라이플 사이클 중 생성될 때만 실행
  // 첫번째 값은 실행할 함수, 두번째 값은 빈 배열
  // 컴포넌트가 생성되자마자 실행하고 싶은 함수 또는 자바스크립트가 있을 때,
  // setInterval과 같이 타이머 함수를 사용해 줄 때,
  // 생성되자마자 실행할 이벤트를 DOM에 추가해주고 싶을 때 사용
  useEffect(() => {
  ...
  // 특정값이 업데이트 될 때마다 화면 업데이트
  // 두번째 값에 특정 state, props가 업데이트 될 때마다 함수가 실행될 수 있도록 작성
  useEffect(() => {
  ...
  return (
    <div>
      <h1>Hook에 관하여</h1>
      <p>
        Hook은 리액트 16.8에서 추가된 버전, 이전 버전에서는 클래스형으로 사용{" "}
        <br />
      ...
      <p>
        ...
        * useState는 비동기로 값을 넣어줌 : 값을 복잡하게 연속적으로 넣어줄 때
        문제가 생길 수도 있다. <br />* 클래스형의 state도 비동기로 들어간다.
      </p>
      <button onClick={changeCount}>setCount 비동기 : {count}</button>
      <button onClick={changeCountFunc}>
        setCount 안에 함수 사용 : {count}
      </button>

      <h3>useEffect</h3>
      <p>
        라이프 사이클을 사용할수 있는 hook <br />
