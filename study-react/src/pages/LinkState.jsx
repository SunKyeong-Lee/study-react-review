...
const LinkState = () => {
  const varInput = "작성한 값";
  
  return (
    <div>
      <h3>Link를 이용해서 값 전달</h3>
      <p>
        LinkState의 path는 /link,
        LocationState의 path는 /location
      </p>
      <p>Link의 속성 state에 값을 넣어서 전달</p>
      <p>Link의 state 대신에 useNavigate의 두번째 인자에 중괄호(객체형태)를 통해서 state:값을 전달할 수 있다.</p>
      ...
      <Link to="/query?id=3&name=안녕">쿼리스트링을 가지고 이동</Link>
    </div>
  );
}
